...
export async function seedSampleContent() {
  try {
    ...
    const REAL_PROJECTS = [
      ...
      { title: "Port of Oakland — Emergency Spill Response or Emergency Repairs", description: "Solicitation notice for emergency spill response or emergency repairs in a hazardous environment at Port of Oakland facilities.", location: "Oakland, California", contactEmail: "Port of Oakland bids/RFP center link in email", postedById: adminId, status: "closed", latitude: "37.8044", longitude: "-122.2712", category: "Bid opportunity", organization: "Port of Oakland", gmailLink: "https://mail.google.com/mail/#all/199a19165dc4f87a", emailSentDate: "2025-10-01", notes: "Contract period: commencing 2026-04-01. Status unknown from email alone." },
      { title: "Port of Oakland — Removal and Disposal of Hazardous Waste and Contaminated Materials", description: "RFP notice for removal and disposal of hazardous waste, universal waste, and other contaminated materials.", location: "Oakland, California", contactEmail: "Port of Oakland bids/RFP center link in email", postedById: adminId, status: "closed", latitude: "37.8044", longitude: "-122.2712", category: "Bid opportunity", organization: "Port of Oakland", gmailLink: "https://mail.google.com/mail/#all/199a18d8203f092d", emailSentDate: "2025-10-01", notes: "Contract period: commencing 2026-04-01. Status unknown from email alone." },
      { title: "C2496256 FIN — MB CVRB Cold Room Remediation", description: "General Building (B) license required. Bidding documents available on 2025-09-30.", location: "UCSF, San Francisco, CA", contactEmail: "Darrick.Lo@ucsf.edu; Karen.Wong@ucsf.edu; BuildingConnected link in email", postedById: adminId, status: "closed", latitude: "37.7631", longitude: "-122.4584", category: "Bid opportunity", organization: "UCSF Real Estate", gmailLink: "https://mail.google.com/mail/#all/199a18c1a71e81ce", emailSentDate: "2025-10-01", notes: "Bidding documents available: 2025-09-30; no bid due date stated." },
      ...
      { title: "SFO T3W Outreach Event", description: "Outreach event related to SFO Terminal 3 West project. See email for attached invitation with full details.", location: "San Francisco International Airport", contactEmail: "See invitation attachment", postedById: adminId, status: "closed", latitude: "37.6213", longitude: "-122.3790", category: "Outreach", organization: "SFO / T3W Project Team", gmailLink: "https://mail.google.com/mail/#all/19953a0cc4557ce4", emailSentDate: "2025-09-16", notes: "Details not stated in the email body; invitation was an attachment." },
    ];

    const existingProjects = await db.select().from(projectOpportunities);
    const isMockData = existingProjects.some(p => p.title.includes("Oakland Unified School District") || p.title.includes("City of Berkeley — Streetscape") || p.title.includes("BART — Platform Safety") || p.title.includes("Affordable Housing Development — East 14th"));
    if (isMockData) {
      await db.delete(projectOpportunities);
      console.log("Removed mock project opportunities");
    }
