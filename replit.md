...
**Visual language (Task #27 — editorial construction journal; Task #33 — mono-font modernization)**: The portal and public site use a custom editorial design system, not stock shadcn. Key tokens live in `client/src/index.css` and `tailwind.config.ts`:
- **Type system (single family, varied by weight + tracking + size)**: Plus Jakarta Sans for everything — body and headlines. `font-display` is now Plus Jakarta Sans at weight 700 with `-0.025em` tracking; `font-numeral` is weight 800 with `-0.04em` tracking + tabular figures for stats / dates / section markers (№01, №02…). The previous Fraunces serif was removed in May 2026 to give the site a modern mobile-app feel — italic accents still work (Plus Jakarta Sans ships italic styles).
...
- **Construction conventions**: section numerals (`№01`, `№02`…) at the top of every public-site section; eyebrow microcopy in uppercase tracked text; pull-quotes use Plus Jakarta Sans italic light with a left gold rule.
- **`responsive-table` utility** (`client/src/index.css`, Task #33): add `responsive-table` to any `<table>` and below `lg` (1024px) it collapses to a stacked card list — `thead` is hidden, each `<tr>` becomes a vertical padded block with cells stacking. Cells hidden on small screens via `hidden md:table-cell` are revealed in the stacked view so no data is lost. `<td className="text-center">` toggle cells use their `title` attribute as the inline label. Used across all 8 admin tables (applications, members, expense budget, revenue funding, SMS contacts + send results, SMS history details, renewal reminders) to eliminate horizontal scroll on phones. Pair with the global `overflow-x: clip` safety net on `body`.
- **Admin tab picker**: the 7-tab admin TabsList is hidden below `lg` and replaced with a full-width shadcn `<Select>` dropdown driven by the same controlled `tab` state — eliminates the horizontal tab scroll on phones.
